...
export interface DocumentRepositoryInterface {
	...
	findByEventId(eventId: number): Promise<unknown[]>
	findById(documentId: number): Promise<unknown | null>
	delete(documentId: number): Promise<unknown>
	update(documentId: number, data: { name?: string }): Promise<unknown>
} 
...
export function RealTimeStatusForm({
	...
	// Watch status to update color automatically
	const watchedStatus = form.watch('status')

	// Update color when status changes
	useEffect(() => {
		const newColor = watchedStatus === 'arrived' ? 'red' : 'green'
		...
		return (
			<div className="flex items-center justify-center py-8">
				<div className="flex items-center space-x-2">
					...
					<span className="text-muted-foreground">Loading event data...</span>
				</div>
			</div>
		)
	}
					...
					<FormField
						...
						render={({ field }) => (
							<FormItem>
								...
								<div className="flex items-center space-x-2">
									<div className={`w-6 h-6 rounded-full ${field.value === 'red' ? 'bg-red-500' : 'bg-green-500'}`} />
									<span className="text-sm text-muted-foreground">
										{field.value === 'red' ? 'Red (Arrived)' : 'Green (Dispatched)'}
									</span>
								</div>
								<FormMessage />
							</FormItem>
